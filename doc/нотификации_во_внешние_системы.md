Навигация: [Главная страница](main_Page.md)/[Система ejudge](система_ejudge.md)/[Использование](использование.md)/[Нотификации во внешние системы](нотификации_во_внешние_системы.md)

Поддерживается с версии [3.11.0](change/изменения_в_версии_3.11.0.md).
...
В настоящее время ejudge поддерживает redis (publish/subscribe) и redis streams в качестве брокеров очередей
сообщений, идентификатор драйвера redis равен `1`, redis streams &mdash; `2`. Если требуется
...
Альтернативно можно можно в конфигурационном файле контеста указать параметр [notification_spec](serve.cfg/global/notification_spec.md). Тогда информация об изменении статуса посылки по умолчанию будет отправляться в заданную очередь.

### Настройка плагинов для очередей сообщений

Для использования redis или redis streams в качестве брокера очередей сообщений необходимо включить в файле [ejudge.xml](ejudge.xml.md) соответствующий плагин:

#### Redis

Внутри тега `plugins` необходимо дописать следующее:

```xml
<plugins>
    ...
    <plugin type="notify" name="redis" load="yes">
        <config>
          <host>127.0.0.1</host>
          <port>6379</port>
          <!-- <password_file>redis_password_file</password_file> -->
        </config>
    </plugin>
</plugins>
```

Здесь `127.0.0.1` и `6379` &mdash; соответственно хост и порт вашего `redis`-сервера. По умолчанию вход будет осуществляться под пользователем `default` без пароля. Вы можете это изменить, создав файл следующего содержания:
...
вписав туда соответственно используемого пользователя, пароль для него, а также хост и порт. Путь до полученного файла необходимо поместить внутрь тега `password_file`. Если передан относительный путь, он (при стандартной настройке) отсчитывается от `/home/judges/data`.

#### Redis streams

Плагин включается аналогично, за исключением того, что в аттрибут `name` тега `plugin` надо передать `redis_streams`.
