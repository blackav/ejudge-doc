...
параметры
тестирования](serve.cfg:tester "wikilink")/[check_cmd](Serve.cfg:tester:check_cmd "wikilink")
...
[`time_limit`](serve.cfg:problem:time_limit "wikilink")) и завершилась
...
переменная [`use_corr`](serve.cfg:problem:use_corr "wikilink") описания
...
- Первый параметр (`argv[1]`) проверяющей программы — путь к файлу, в
  ...
  [`test_dir`](serve.cfg:problem:test_dir "wikilink")), а не к файлу,
  ...
  [`is_dos`](serve.cfg:tester:is_dos "wikilink") проверяющая программа
...
- Если установлена конфигурационная переменная
  [`use_corr`](serve.cfg:problem:use_corr "wikilink"), третий параметр
  (`argv[3]`) проверяющей программы — путь к файлу, содержащему
  правильный ответ на текущий тест (см. переменную
  [`corr_dir`](serve.cfg:problem:corr_dir "wikilink")). Если
